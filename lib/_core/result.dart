...
import 'package:template/_core/failure.dart';
...
class Result<T> with _$Result<T> {
  ...
  const factory Result.success({required T data}) = ResSuccess<T>;

  const factory Result.failure({required Failure error}) = ResFailure<T>;

  static Result<T> guard<T>(T Function() body) {
    try {
      return Result.success(data: body());
    } on Exception catch (e) {
      return Result.failure(error: Failure(e));
    }
  }
  ...
  void ifFailure(Function(Failure e) body) {
    maybeWhen(
      failure: (e) => body(e),
      orElse: () {
        // no-op
...
extension ResultObjectExt<T> on T {
  ...
  Result<T> asFailure(Exception e) => Result.failure(error: Failure(e));
}
